...
type CancellationDetailDialogProps = {
  data: Reservation;
...
export default function CancellationDetailDialog({
  data, 
  trigger, 
  open, 
  onOpenChange 
...
function convertPaymentStatus(status: string) {
  if (status === 'canceled') return 'Pembatalan Berhasil';
  if (status === 'waiting') return 'Menunggu Refund';
  return status;
}
